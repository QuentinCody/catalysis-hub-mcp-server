...
var MyMCP = class extends McpAgent {
  constructor() {
    ...
    this.GRAPHQL_ROOT = "http://api.catalysis-hub.org/graphql";
  }
  static {
    __name(this, "MyMCP");
  }
  ...
  async executeGraphQLQuery(query, variables) {
    try {
      ...
      console.error(`Error during Catalysis Hub request: ${error3 instanceof Error ? error3.message : String(error3)}`);
      if (error3 instanceof TypeError || error3 instanceof Error) {
        return {
          errors: [{
            message: `HTTP Request Error connecting to Catalysis Hub: ${error3.message}`
          }]
        };
      }
      return {
        errors: [{
          message: `An unexpected error occurred: ${String(error3)}`
        }]
      };
    }
  }
...
var src_default = {
  fetch(request, env2, ctx) {
    const id = env2.MCP_OBJECT.idFromName("catalysis-hub");
...
// .wrangler/tmp/bundle-bFVpKJ/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-bFVpKJ/middleware-loader.entry.ts
